/*	Exercise 3.8.7 Interger Programming, L. Wosey 
...
class Graph {
	...
	public:
		...
		void read_file() {
			if (freopen("3.8.7_data", "r", stdin) == NULL) {
...
int main(int argc, char *argv[]) {
	...
	try {
		...
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < n; j++) {
				x[i][j] = model.addVar(0.0, 1.0, G.adjm[i][j],
				GRB_BINARY, "x_"+to_string(i)+"_"+to_string(j));
			}
		}

		for (int i = 0; i < n; i++)
			y[i] = model.addVar(0.0, 1.0, 0.0, GRB_BINARY, "y_" + to_string(i));
		...
		y[0].set(GRB_DoubleAttr_UB, 0);
		y[n - 1].set(GRB_DoubleAttr_LB, 1);

		// Constraints

		for (int i = 0; i < n; i++)
			for (auto a : G.get_adj(i))
				model.addConstr(y[i] - y[a.first] + x[i][a.first] >= 0, "constr_" + to_string(i) + "_" + to_string(a.first));
		...
		for (int i = 0; i < n; i++){
			cout << i << ' ';
			for (int j = 0; j < n; j++)
				cout << abs(x[i][j].get(GRB_DoubleAttr_X)) << ' ';
		cout<<endl;   
		}
