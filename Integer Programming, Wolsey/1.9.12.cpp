/*	Exercise 1.9.12 Interger Programming, L. Wosey 
...
int main(int argc, char *argv[]) {
	...
	try {
		...
		for (long long mask = 1; mask < (1ll << n) - 1; mask++) {
			if (__builtin_popcountll(mask) < 2 || __builtin_popcountll(mask) >= n)
				continue;
			...
			int size = 2*(__builtin_popcountll(mask) - 1);      
			model.addConstr(expr <= size, "subtour_" + to_string(mask));      
		}
